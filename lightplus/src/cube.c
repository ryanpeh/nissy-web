...
read_invtables_file(void)
{
	...
	if (nissy_stream_enabled("invtables")) {
		uint64_t off = 0, n;
		int b2 = sizeof(uint16_t);
		unsigned int e2 = BINOM12ON4*FACTORIAL4, c2 = FACTORIAL8,
		    p2 = FACTORIAL8, s2 = FACTORIAL6;
		for (ui = 0; ui < POW2TO11; ui++) {
			n = (uint64_t)e2*b2;
			if (!nissy_stream_read("invtables", eo_invtable_e[ui], off, n)) return false; off += n;
			if (!nissy_stream_read("invtables", eo_invtable_m[ui], off, n)) return false; off += n;
			if (!nissy_stream_read("invtables", eo_invtable_s[ui], off, n)) return false; off += n;
		}
		for (ui = 0; ui < POW3TO7; ui++) {
			n = (uint64_t)c2*b2;
			if (!nissy_stream_read("invtables", co_invtable[ui], off, n)) return false; off += n;
		}
		n = (uint64_t)p2*b2;
		if (!nissy_stream_read("invtables", cp_invtable, off, n)) return false; off += n;
		n = (uint64_t)s2*b2;
		if (!nissy_stream_read("invtables", cpos_invtable, off, n)) return false; off += n;
		return true;
	}
#endif
