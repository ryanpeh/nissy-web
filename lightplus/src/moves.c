...
read_mtables_file(void)
{
	...
	if (nissy_stream_enabled("mtables")) {
		uint64_t off = 0, n;
#define LP_R(dst, idx) do { n = me[idx]*b; \
	if (!nissy_stream_read("mtables", dst, off, n)) return false; off += n; } while (0)
		for (m = 0; m < NMOVES; m++) {
			LP_R(epose_mtable[m],0); LP_R(eposs_mtable[m],1); LP_R(eposm_mtable[m],2);
			LP_R(eofb_mtable[m],3);  LP_R(eorl_mtable[m],4);  LP_R(eoud_mtable[m],5);
			LP_R(cp_mtable[m],6);    LP_R(coud_mtable[m],7);  LP_R(corl_mtable[m],8);
			LP_R(cofb_mtable[m],9);  LP_R(cpos_mtable[m],10);
		}
#undef LP_R
		return true;
	}
