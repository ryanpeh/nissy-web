...
genptable(PruneData *pd, int nthreads)
{
	...
	/* Allocate only the compact size first: loading a precomputed table
	 * (the common case, and the only one in the browser) needs just that.
	 * Grow to the uncompressed size (2x for compact) only if we must
	 * actually generate the table. This roughly halves peak memory when
	 * loading pd_nxopt31_HTM (2.47 GB instead of 4.93 GB). */
	sz = ptablesize(pd);
	...
	if (pd->compact) {
		sz = ptablesize(pd) * 2;
		pd->ptable = realloc(pd->ptable, sz * sizeof(entry_group_t));
	}
...
read_ptable_file(PruneData *pd)
{
	...
	if (nissy_stream_enabled(pd->filename)) {
		unsigned char hdr[4 + 16 * 8];
		uint64_t sz = ptablesize(pd) * sizeof(entry_group_t);
		if (!nissy_stream_read(pd->filename, hdr, 0, sizeof(hdr)))
			return false;
		memcpy(&pd->base, hdr, sizeof(int));
		memcpy(pd->count, hdr + sizeof(int), 16 * sizeof(uint64_t));
		if (!nissy_stream_read(pd->filename, pd->ptable, sizeof(hdr), sz))
			return false;
		return true;
	}
#endif
