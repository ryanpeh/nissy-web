...
uint64_t    move_eofbepos_16[NMOVES][CLASSES_EOFBEPOS_16];
...
Trans       ttrep_move_eofbepos_16[NMOVES][CLASSES_EOFBEPOS_16];
...
read_symdata_file(SymData *sd)
{
	...
	if (nissy_stream_enabled(sd->filename)) {
		uint64_t off = 0;
		if (!nissy_stream_read(sd->filename, &sd->sym_coord->max, 0,
		    sizeof(uint64_t))) return false;
		off = sizeof(uint64_t);
		n = sd->coord->max;
		if (!nissy_stream_read(sd->filename, sd->unsym, off,
		    (*sn)*sizeof(uint64_t))) return false;
		off += (*sn)*sizeof(uint64_t);
		if (!nissy_stream_read(sd->filename, sd->selfsim, off,
		    (*sn)*sizeof(uint64_t))) return false;
		off += (*sn)*sizeof(uint64_t);
		if (!nissy_stream_read(sd->filename, sd->class, off,
		    n*sizeof(uint64_t))) return false;
		off += n*sizeof(uint64_t);
		if (!nissy_stream_read(sd->filename, sd->transtorep, off,
		    n*sizeof(Trans))) return false;
		return true;
	}
#endif
...
#ifdef NISSY_WASM_STREAM
	if (nissy_stream_enabled("symc_moves")) {
		uint64_t off = 0, n;
		for (m = 0; m < NMOVES; m++) {
			n = (uint64_t)CLASSES_CP_16*sizeof(uint64_t);
			if (!nissy_stream_read("symc_moves", move_cp_16[m], off, n)) return false; off += n;
			n = (uint64_t)CLASSES_EOFBEPOS_16*sizeof(uint64_t);
			if (!nissy_stream_read("symc_moves", move_eofbepos_16[m], off, n)) return false; off += n;
			n = (uint64_t)CLASSES_CP_16*sizeof(Trans);
			if (!nissy_stream_read("symc_moves", ttrep_move_cp_16[m], off, n)) return false; off += n;
			n = (uint64_t)CLASSES_EOFBEPOS_16*sizeof(Trans);
			if (!nissy_stream_read("symc_moves", ttrep_move_eofbepos_16[m], off, n)) return false; off += n;
		}
		return true;
	}
#endif
...
#ifdef NISSY_WASM_STREAM
	if (nissy_stream_enabled("symc_trans")) {
		uint64_t off = 0, n;
		for (t = 0; t < NTRANS; t++) {
			n = (uint64_t)POW2TO11*BINOM12ON4*sizeof(int);
			if (!nissy_stream_read("symc_trans", trans_eofbepos[t], off, n)) return false; off += n;
			n = (uint64_t)FACTORIAL8*sizeof(int);
			if (!nissy_stream_read("symc_trans", trans_epud[t], off, n)) return false; off += n;
			n = (uint64_t)BINOM8ON4*sizeof(int);
			if (!nissy_stream_read("symc_trans", trans_cpud_separate[t], off, n)) return false; off += n;
		}
		return true;
	}
#endif
