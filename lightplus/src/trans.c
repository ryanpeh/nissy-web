...
read_ttables_file(void)
{
	...
	if (nissy_stream_enabled("ttables")) {
		uint64_t off = 0, n;
#define LP_R(dst, idx) do { n = me[idx]*b; \
	if (!nissy_stream_read("ttables", dst, off, n)) return false; off += n; } while (0)
		for (m = 0; m < NTRANS; m++) {
			LP_R(epose_ttable[m],0); LP_R(eposs_ttable[m],1); LP_R(eposm_ttable[m],2);
			LP_R(eo_ttable[m],3);    LP_R(cp_ttable[m],4);    LP_R(co_ttable[m],5);
			LP_R(cpos_ttable[m],6);  LP_R(moves_ttable[m],7);
		}
#undef LP_R
		return true;
	}
